...
function ArrayChallenge(strArr) {
  ...
  for (var pair of strArr) {
    var [child, parent] = pair.slice(1, -1).split(",").map(Number);
    if (varOcg.has(parent) && varOcg.get(parent).length === 2) {
      return "false";
    }
    varOcg.set(parent, (varOcg.get(parent) || []).concat(child));
    roots.delete(child);
    roots.add(parent);
  }

  return roots.size === 3 ? "true" : "false";
}

// Test the function with the provided examples
console.log(ArrayChallenge(["(1,2)", "(2,4)", "(5,7)", "(7,2)", "(9,5)"])); // Expected output: 'true'
console.log(ArrayChallenge(["(1,2)", "(3,2)", "(2,12)", "(5,2)"])); // Expected output: 'false'
